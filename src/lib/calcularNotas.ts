// src/lib/calcularNotas.ts
// Lógica central de /api/notas/calcular, extraída a una función reutilizable
// para que tanto el endpoint de un solo estudiante como el de lote (varios
// estudiantes a la vez) compartan exactamente el mismo cálculo, sin duplicar
// código ni hacer una petición HTTP por estudiante en el caso de lote.
...
// Calcula (y guarda en resumen_libro / resumen_etapa) el avance de una
// inscripción, y aplica el auto-completado a 'completada' cuando ya se
// registraron todas las notas de tareas y exámenes de todos sus libros.
export async function calcularInscripcion(
  inscripcion_id: string,
  numero_libro?: number | string
  ...
  if (!insc) {
    return { ok: false, error: 'Inscripción no encontrada', resultados: [], inscripcion_completada: false, estado_final: null }
  }

  const nums = numero_libro ? [parseInt(String(numero_libro))] : [1, 2]
  ...
  // ── AUTO-COMPLETADO ──────────────────────────────────────────────────
  // Siempre 'completada' aquí (nunca 'finalizada' directamente); el paso a
  // 'finalizada' es perezoso, ver src/lib/estadoInscripcion.ts.
  ...
  return { ok: true, resultados, inscripcion_completada, estado_final }
}
