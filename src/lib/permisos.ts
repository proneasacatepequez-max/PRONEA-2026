...
// La sede ES la institución — se pasa sede_id como p_institucion_id al RPC
export async function visibilidadInstitucion(sedeId: string) {
  const { data } = await supabaseAdmin.rpc('verificar_visibilidad_coordinador', { p_institucion_id: sedeId })
