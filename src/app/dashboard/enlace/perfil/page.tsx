'use client'
// src/app/dashboard/enlace/perfil/page.tsx — NUEVA PÁGINA
...
export default function EnlacePerfilPage() {
  ...
  return (
    <div className="ap">
      <header className="topbar"><div className="page-title">👤 Mi Perfil — Enlace Institucional</div></header>
      <div className="pc max-w-2xl space-y-5">
        {msg && <div className={`alert ${msg.startsWith('✅') ? 'al-s' : 'al-e'}`}>{msg}</div>}

        <div className="card">
          <div className="card-title">🔗 Datos del Enlace</div>
          <div className="grid grid-cols-2 gap-x-6 gap-y-3 text-sm">
            {[
              ...
              ['Tipo institución', (p?.institucion as any)?.tipo ?? '—'],
              ['Municipio',        (p?.institucion as any)?.municipio?.nombre ?? '—'],
