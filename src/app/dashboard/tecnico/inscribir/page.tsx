...
// REDISEÑO COMPLETO: primero buscar, luego decidir "nuevo registro" o "reinscribir existente"
...
export default function InscribirEstudiantePage() {
  ...
  const [ultimaInscripcion, setUltimaInscripcion] = useState<any>(null)
  ...
  // Form inscripción (común a nuevo y reinscribir)
  ...
  const empezarNuevo = () => {
    setEst({
      primer_nombre: buscarQ.length >= 3 && !buscarQ.match(/^\d+$/) ? '' : '',
  ...
  const inscribirNuevo = async () => {
    ...
    try {
      ...
      if (!resEst.ok) {
        flash('❌ ' + (dEst.error ?? 'Error al crear estudiante'))
  ...
  const confirmarInscripcion = async (estudianteId: string) => {
    try {
      const sedeSel = sedes.find((s: any) => s.id === insc.sede_id)
      const etapaSel = etapas.find((e: any) => String(e.id) === String(insc.etapa_id))
      ...
      const resInsc = await fetch('/api/inscripciones', {
        ...
        body: JSON.stringify({
          ...
          // tecnico_id ya NO es necesario — la API lo resuelve automáticamente
      ...
      let dInsc: any = {}
      try { dInsc = await resInsc.json() } catch { /* no json */ }

      if (resInsc.status === 409) {
        // Ya inscrito — no es error real, mostrar info útil
        flash(`ℹ️ ${dInsc.error ?? 'Este estudiante ya tiene una inscripción activa en esta etapa'}`)
        setSaving(false)
        return
      }

      if (!resInsc.ok) {
        flash('❌ ' + (dInsc.error ?? 'Error al inscribir'))
        setSaving(false)
        return
      }

      // Éxito — guardar datos para la pantalla de confirmación
      setUltimaInscripcion({
        nombre:    estudianteSel
          ? `${estudianteSel.primer_nombre} ${estudianteSel.primer_apellido}`
          : `${est.primer_nombre} ${est.primer_apellido}`,
        etapa:     etapaSel?.nombre ?? `Etapa ${insc.etapa_id}`,
        sede:      sedeSel?.nombre  ?? 'la sede seleccionada',
        ciclo:     insc.ciclo_escolar,
        version:   insc.version_libro,
      })
      setModo('exito')
    } catch (e: any) {
      flash('❌ Error inesperado: ' + (e?.message ?? ''))
    ...
    <div className="ap">
      ...
      <div className="pc max-w-lg text-center">
        <div className="card py-10">
          ...
          <h2 className="text-xl font-extrabold text-green-600 mb-3">¡Inscripción registrada!</h2>

          {ultimaInscripcion ? (
            <div className="bg-green-50 border border-green-200 rounded-xl p-4 mb-5 text-left space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-gray-500 font-semibold">Estudiante</span>
                <span className="text-gray-800 font-bold">{ultimaInscripcion.nombre}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500 font-semibold">Etapa</span>
                <span className="text-gray-800">{ultimaInscripcion.etapa}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500 font-semibold">Sede</span>
                <span className="text-gray-800">🏫 {ultimaInscripcion.sede}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-gray-500 font-semibold">Libro</span>
                <span className="text-gray-800">
                  {ultimaInscripcion.version === 'nuevo' ? '📗 Nuevo' : '📙 Viejo'} · Ciclo {ultimaInscripcion.ciclo}
                </span>
              </div>
            </div>
          ) : (
            <p className="text-gray-500 text-sm mb-5">
              {estudianteSel
                ? `${estudianteSel.primer_nombre} ${estudianteSel.primer_apellido} fue inscrito exitosamente.`
      ...
      <header className="topbar">
        <div>
          ...
          <div className="text-xs text-gray-400">
            {modo === 'buscar' ? 'Paso 1: Buscar si ya está registrado' : modo === 'nuevo' ? 'Nuevo registro estudiantil' : 'Avanzar de etapa'}
          </div>
        </div>
      </header>
        ...
        {modo === 'buscar' && (
          <div className="card">
            ...
            <div className="text-sm text-gray-500 mb-4">
              Busca por nombre, CUI o código MINEDUC antes de crear un registro nuevo.
              Si el estudiante ya cursó una etapa, aquí lo encontrarás para avanzarlo
              a la siguiente sin duplicar sus datos.
            </div>

            <input
              className="inp"
              placeholder="Escribe nombre, apellido, CUI (13 dígitos) o código MINEDUC..."
              value={buscarQ}
              ...
              <div className="mt-4 space-y-2">
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <div className="text-xs text-gray-400 mt-0.5">
                          ...
                          {r.cui && <span className="ml-2 font-mono">CUI: {r.cui}</span>}
                          {r.cui_pendiente && <span className="ml-2 text-orange-500">CUI pendiente</span>}
                        </div>
                      </div>
                      <div className="text-right">
                        {r.ultima_etapa && (
                          <span className="badge badge-blue text-xs">Última: {r.ultima_etapa.nombre}</span>
                        )}
                        {r.inscripcion_activa && (
                          <div className="text-xs text-green-600 mt-1">✓ Inscripción activa</div>
                        )}
                      </div>
                    </div>
                  </button>
                ...
                <div className="text-center pt-2">
                  <button className="btn btn-g text-sm" onClick={empezarNuevo}>
                    Ninguno de estos — ＋ Crear nuevo registro
                  </button>
                </div>
              </div>
            )}
            ...
            {buscarQ.trim().length > 0 && buscarQ.trim().length < 3 && (
              <div className="text-xs text-gray-400 mt-2">Escribe al menos 3 caracteres para buscar...</div>
            )}
          </div>
        )}
        ...
        {modo === 'reinscribir' && estudianteSel && (
          <div className="card">
            ...
            <div className="bg-blue-50 rounded-xl p-4 mb-4">
              <div className="font-bold text-gray-800">
                {estudianteSel.primer_nombre} {estudianteSel.segundo_nombre} {estudianteSel.primer_apellido} {estudianteSel.segundo_apellido}
              </div>
              <div className="text-sm text-gray-500 mt-1">
                {estudianteSel.codigo_estudiante && <span className="font-mono">{estudianteSel.codigo_estudiante}</span>}
                {estudianteSel.cui && <span className="ml-2 font-mono">CUI: {estudianteSel.cui}</span>}
              </div>
              <div className="text-sm text-gray-500">{edad(estudianteSel.fecha_nacimiento)} · {estudianteSel.genero}</div>
              ...
              {estudianteSel.inscripcion_activa && (
                <div className="text-xs text-orange-500 mt-1">
                  ⚠️ Ya tiene una inscripción activa en: {estudianteSel.inscripcion_activa.etapa?.nombre}
                </div>
              )}
            </div>
        ...
        {modo === 'nuevo' && (
          <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="fg">
                    ...
                    <input className="inp font-mono" value={est.cui} onChange={E('cui')}
                      disabled={est.cui_pendiente} placeholder={est.cui_pendiente ? 'Pendiente' : '1234567890123'} maxLength={13} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3 items-center">
                  <label className="flex items-center gap-2 text-sm cursor-pointer">
                    <input type="checkbox" checked={est.cui_pendiente}
                      onChange={e => setEst(p => ({ ...p, cui_pendiente: e.target.checked, cui:'' }))} className="w-4 h-4" />
            ...
            <div className="card">
              ...
              <div className="flex justify-between mt-4">
                <button className="btn btn-g" onClick={resetTodo}>← Buscar otro</button>
                <button className="btn btn-p" onClick={inscribirNuevo} disabled={saving}>
                  {saving
