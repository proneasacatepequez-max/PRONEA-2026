...
// COMPLETO: tabla con notas por área, botón asignar estudiantes, buscador
import { useState, useEffect, useCallback } from 'react'

// Nombres de áreas por etapa
const AREA_NOMBRE_ETAPA: Record<string, Record<string, string>> = {
  'BA4': {
    'PROD': 'Gestión de Proyectos',
...
const nombreArea = (codigo: string, etapaCodigo?: string) => {
  if (etapaCodigo && AREA_NOMBRE_ETAPA[etapaCodigo]?.[codigo]) {
    return AREA_NOMBRE_ETAPA[etapaCodigo][codigo]
  }
  const defaults: Record<string, string> = {
    ...
    'CS':   'Ciencias Sociales',
    'CL':   'Comunicación y Lenguaje',
    'PROD': 'Productividad y Desarrollo',
  }
  return defaults[codigo] ?? codigo
}
...
export default function TecnicoSireexPage() {
  ...
  const abrirMiembros = (g: any) => {
    setGrupoActivo(g)
    setModal('miembros')
  }

  const guardar = async () => {
    if (modal === 'crear' && (!form.etapa_id || !form.sede_id)) {
      flash('❌ Etapa y sede son requeridos'); return
    }
    setSaving(true)
  ...
  const asignarSeleccionados = async () => {
    ...
    const res = await fetch('/api/sireex/estudiantes', {
      ...
      body: JSON.stringify({
        grupo_sireex_id: grupoActivo.id,
        inscripcion_ids: [...seleccionados],
      }),
    })
    const d = await res.json()
    flash(res.ok ? `✅ ${d.agregados} estudiante(s) asignados al grupo` : '❌ ' + (d.error ?? 'Error'))
    if (res.ok) {
      setSeleccionados(new Set())
      // Recargar miembros y disponibles
  ...
  const removerMiembro = async (membId: string) => {
    if (!confirm('¿Remover este estudiante del grupo?')) return
    const res = await fetch(`/api/sireex/estudiantes?id=${membId}`, { method: 'DELETE' })
    if (res.ok) {
      setMiembros(prev => prev.filter((m: any) => m.id !== membId))
      flash('✅ Estudiante removido del grupo')
      await cargar()
    } else flash('❌ Error al remover')
  }
  ...
  const toggleEstado = async (g: any) => {
    const nuevo = g.estado === 'abierto' ? 'cerrado' : 'abierto'
    const res = await fetch('/api/sireex/grupos', {
      method: 'PATCH', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: g.id, estado: nuevo }),
    })
    if (res.ok) { await cargar(); flash(`✅ Grupo ${nuevo}`) }
  }

  const F = (k: string) => (e: React.ChangeEvent<HTMLInputElement|HTMLSelectElement|HTMLTextAreaElement>) =>
    setForm(p => ({ ...p, [k]: e.target.value }))

  const BADGE: Record<string, string> = { abierto:'badge-green', cerrado:'badge-yellow', exportado:'badge-blue' }

  const etapaCodigo = (grupoActivo?.etapa as any)?.codigo
    ...
    <div className="ap">
      ...
      <div className="pc">
        ...
        <div className="alert al-i mb-4 text-sm">
          📋 Crea un grupo por etapa, asigna los estudiantes, ingresa el código MINEDUC cuando lo tengas y descarga el Excel con notas.
        </div>
                    ...
                    <tr key={g.id} className={`border-b hover:bg-blue-50 ${idx%2===0?'bg-white':'bg-sky-50/30'}`}>
                      ...
                      <td className="px-3 py-2.5">
                        <div className="flex gap-1 flex-nowrap">
                          ...
                          <button className="btn btn-s btn-sm" onClick={() => abrirMiembros(g)} title="Asignar estudiantes">
                            👥
                          </button>
          ...
          <div className="min-h-full flex items-start justify-center p-4 pt-8">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl">
              <div className="flex items-center justify-between px-6 py-4 border-b">
                <div>
                  <h3 className="text-base font-extrabold">
                    👥 Estudiantes — {grupoActivo.codigo}
              ...
              {loadMiembros ? (
                ...
                <div className="px-6 py-5 space-y-5">
                  ...
                  <div className="card bg-blue-50/50 border border-blue-100">
                    <div className="card-title text-blue-700 mb-3">➕ Agregar estudiantes de la misma etapa</div>
                    <div className="flex gap-2 mb-3">
                      <input className="inp flex-1" placeholder="🔍 Buscar por nombre, código o CUI..."
                        value={buscarEst} onChange={e => setBuscarEst(e.target.value)} />
                      <button className="btn btn-p" onClick={asignarSeleccionados} disabled={asignando || seleccionados.size === 0}>
                        {asignando
                          ? <span className="flex items-center gap-2"><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Asignando...</span>
                    ...
                    {dispFiltrados.length === 0 ? (
                      <div className="text-sm text-gray-400 text-center py-3">
                        {disponibles.length === 0
                          ? 'Todos los estudiantes de esta etapa ya están en el grupo'
                      ...
                      <div className="max-h-48 overflow-y-auto">
                        <table className="w-full text-sm">
                          <thead className="sticky top-0 bg-blue-50">
                            <tr>
                              <th className="px-2 py-1 w-8"><input type="checkbox"
                              ...
                              <th className="px-2 py-1 text-left text-xs font-bold">Código MINEDUC</th>
                            ...
                            {dispFiltrados.map((i: any) => {
                              ...
                              return (
                                <tr key={i.id} className="border-b hover:bg-blue-50/50">
                                  ...
                                  <td className="px-2 py-1 text-sm font-semibold whitespace-nowrap">
                  ...
                  {/* Tabla de miembros con notas */}
                  <div>
                    <div className="card-title mb-3">
                      📋 Estudiantes en el grupo ({miembros.length})
                    </div>
                    {miembros.length === 0 ? (
                      <div className="text-center py-6 text-gray-400 text-sm">
                        Sin estudiantes asignados. Usa el buscador de arriba para agregar.
                      </div>
                    ) : (
                      <div className="overflow-x-auto">
                        <table className="w-full text-xs border-collapse min-w-[1100px]">
                          <thead>
                            <tr className="bg-gradient-to-r from-blue-800 to-blue-900 text-white">
                              <th className="px-2 py-2 text-left whitespace-nowrap border-r border-blue-700">Código MINEDUC</th>
                              <th className="px-2 py-2 text-left whitespace-nowrap border-r border-blue-700">CUI</th>
                              <th className="px-2 py-2 text-left whitespace-nowrap border-r border-blue-700">Nombre Completo</th>
                              <th className="px-2 py-2 text-left whitespace-nowrap border-r border-blue-700">Etapa</th>
                              {areas.map((a: any) => (
                                <th key={a.id} className="px-2 py-2 text-center whitespace-nowrap border-r border-blue-700 text-xs">
                                  {nombreArea(a.codigo, etapaCodigo)}
                                  <div className="font-normal text-blue-200 text-xs">/50</div>
                                </th>
                              ))}
                              <th className="px-2 py-2 text-center whitespace-nowrap border-r border-blue-700">Total</th>
                              <th className="px-2 py-2 text-center whitespace-nowrap border-r border-blue-700">Estado</th>
                          ...
                          <tbody>
                            {miembros.map((m: any, idx: number) => {
                              ...
                              const total = areas.reduce((acc: number, a: any) => {
                                const c = a.codigo ?? a.nombre
                                return acc + (notas[c]?.total ?? 0)
                              }, 0)
                              ...
                              return (
                                <tr key={m.id}
                                  className={`border-b ${idx%2===0?'bg-white':'bg-sky-50/30'} hover:bg-blue-50`}>
                                  <td className="px-2 py-2 font-mono font-bold text-blue-700 whitespace-nowrap">
                                    {e?.codigo_estudiante ?? '—'}
                                  </td>
                                  <td className="px-2 py-2 font-mono whitespace-nowrap">{e?.cui ?? '—'}</td>
                                  ...
                                  {areas.map((a: any) => {
                                    ...
                                    return (
                                      <td key={a.id} className={`px-2 py-2 text-center font-bold ${
                                        val === null || val === undefined ? 'text-gray-300'
                                        : val >= 30 ? 'text-green-600' : 'text-red-500'
                                      }`}>
                                  ...
                                  <td className="px-2 py-2 text-center">
                                    {pendiente
                                      ? <span className="badge badge-yellow text-xs">⏳</span>
                                      : promovido
              ...
              <div className="flex justify-between px-6 py-4 border-t bg-gray-50 rounded-b-2xl">
                <button className="btn btn-g" onClick={() => exportarExcel(grupoActivo.id, grupoActivo.codigo)}>
                  ⬇️ Descargar Excel con notas
                </button>
