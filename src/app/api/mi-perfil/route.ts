...
// FIX: consultas defensivas que no fallan cuando faltan datos relacionados
// El perfil se muestra aunque falten municipio, departamento u otros
...
export async function GET(req: NextRequest) {
  ...
  if (s.rol === 'tecnico') {
    // Consulta en dos pasos: primero datos básicos, luego joins opcionales
    ...
    if (error || !tec) {
      // El usuario existe pero no tiene perfil de técnico creado
      return ok({
        rol: 'tecnico',
        perfil: null,
        _aviso: 'Tu perfil de técnico no está configurado. Contacta al administrador.',
      })
    }

    // Obtener datos relacionados por separado (sin fallar si no existen)
    const [usu, deptoData, muniData, sedesData] = await Promise.allSettled([
      supabaseAdmin.from('usuarios')
        .select('correo, ultimo_acceso, primer_ingreso')
        .eq('id', s.sub).single(),
      tec.departamento_id
        ? supabaseAdmin.from('departamentos').select('id, nombre').eq('id', tec.departamento_id).single()
        : Promise.resolve({ data: null }),
      tec.municipio_id
        ? supabaseAdmin.from('municipios').select('id, nombre').eq('id', tec.municipio_id).single()
        : Promise.resolve({ data: null }),
      supabaseAdmin.from('tecnico_sedes')
  ...
  if (s.rol === 'director') {
    ...
    const [usu, sede, muni] = await Promise.allSettled([
      supabaseAdmin.from('usuarios').select('correo, ultimo_acceso').eq('id', s.sub).single(),
      supabaseAdmin.from('directores').select('sede:sedes(id, nombre, municipio:municipios(nombre))').eq('usuario_id', s.sub).single(),
      dir.municipio_id ? supabaseAdmin.from('municipios').select('id, nombre').eq('id', dir.municipio_id).single() : Promise.resolve({ data: null }),
    ])
    ...
    return ok({
      ...
      perfil: {
        ...dir,
        usuario:  usu.status === 'fulfilled'  ? usu.value.data  : null,
        sede:     sede.status === 'fulfilled'  ? (sede.value as any).data?.sede  : null,
  ...
  if (s.rol === 'enlace_institucional') {
    ...
    const [usu, muni] = await Promise.allSettled([
      supabaseAdmin.from('usuarios').select('correo, ultimo_acceso').eq('id', s.sub).single(),
      enl.municipio_id ? supabaseAdmin.from('municipios').select('id, nombre').eq('id', enl.municipio_id).single() : Promise.resolve({ data: null }),
    ])
...
export async function PATCH(req: NextRequest) {
  ...
  if (b.contrasena_actual !== undefined) {
    ...
    const { data: u } = await supabaseAdmin
      .from('usuarios').select('contrasena_hash').eq('id', s.sub).single()
  ...
  for (const campo of camposComunes) {
    if (b[campo] !== undefined) upd[campo] = b[campo] === '' ? null : b[campo]
  }
  if (upd.municipio_id) upd.municipio_id = parseInt(String(upd.municipio_id))

  // Estudiante: campos especiales
  if (s.rol === 'estudiante') {
  ...
  if (s.rol === 'administrador') {
    const camposEstab = ['director_nombre','director_titulo','telefono','whatsapp','correo',
      'nombre_completo','nombre_corto','municipio','departamento','direccion',
      'horario_atencion','facebook','sitio_web']
    const updEstab: any = {}
