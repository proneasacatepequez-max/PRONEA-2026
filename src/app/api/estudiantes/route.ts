...
// FIX CRÍTICO: verificación de CUI duplicado no dispara con CUI vacío o pendiente
...
export async function POST(req: NextRequest) {
  ...
  const {
    ...
    cantidad_hijos = 0, posee_enfermedad = false, descripcion_enfermedad,
    toma_medicamento = false, alergias, personas_vivienda,
    posee_internet = false, posee_computadora = false, observaciones_generales,
  } = b
  ...
  // ─────────────────────────────────────────────────────────
  // FIX CRÍTICO: el CUI se valida SOLO si:
  //   1. cui_pendiente es FALSE
  //   2. cui tiene valor real (no vacío, no null, no solo espacios)
  //   3. tiene al menos 5 caracteres (evita búsquedas accidentales)
  // ─────────────────────────────────────────────────────────
  const cuiLimpio = cui?.trim() ?? ''
  const cuiFinal  = !cui_pendiente && cuiLimpio.length > 0 ? cuiLimpio : null

  if (cuiFinal && cuiFinal.length > 0) {
    // Verificar CUI duplicado SOLO con valor real
    ...
    if (dup) {
      return err(
        `Ya existe un estudiante registrado con ese CUI: ${dup.primer_nombre} ${dup.primer_apellido} (${dup.codigo_estudiante})`,
        409
      )
    }
  }

  // Código MINEDUC es editable — usar el que venga del formulario o generar uno temporal
  ...
  if (eU) {
    // Correo duplicado — generar uno alternativo
    ...
    if (eU2) return err('Error al crear usuario: ' + eU2.message, 500)
    v_uid = usu2.id
  } else {
    v_uid = usu.id
  }
  ...
  if (error) return err('Error al crear estudiante: ' + error.message, 500)
