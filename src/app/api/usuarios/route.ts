...
// FIX: tecnicos.cui NOT NULL UNIQUE — se genera automáticamente
...
export async function GET(req: NextRequest) {
  ...
  const { data, error } = await supabaseAdmin.from('usuarios')
    .select(`id,correo,rol,activo,ultimo_acceso,creado_en,primer_ingreso,
      tecnicos(id,primer_nombre,primer_apellido,telefono,codigo_tecnico,cui),
...
export async function POST(req: NextRequest) {
  ...
  const{correo,contrasena,rol,primer_nombre,primer_apellido,
        segundo_nombre='',segundo_apellido='',telefono='',codigo_tecnico=''}=b
  ...
  try {
    if(rol==='tecnico'){
      ...
      // cui: NOT NULL UNIQUE — generar automáticamente si no viene
      const cui=b.cui?.trim()||`CUI-${Date.now()}-${Math.floor(Math.random()*9000+1000)}`
      await supabaseAdmin.from('tecnicos').insert({
        usuario_id:usu.id, primer_nombre:primer_nombre.trim(),
        segundo_nombre:segundo_nombre?.trim()||null,
        primer_apellido:primer_apellido.trim(),
        segundo_apellido:segundo_apellido?.trim()||null,
        telefono:telefono||null, codigo_tecnico:codigo,
        cui,  // ← requerido NOT NULL UNIQUE
        activo:true,
      })
      ...
      // enlace requiere institucion_id — usar primera institución activa como fallback
      let instId=b.institucion_id
      if(!instId){const{data:inst}=await supabaseAdmin.from('instituciones').select('id').eq('activo',true).limit(1).single();instId=inst?.id}
      if(instId){
        await supabaseAdmin.from('enlaces_institucionales').insert({
          ...
          primer_apellido:primer_apellido.trim(),
          segundo_apellido:segundo_apellido?.trim()||null,
          telefono:telefono||null, institucion_id:instId, activo:true,
        })
      }
      ...
      try{
        await supabaseAdmin.from('coordinadores_departamento').insert({
          usuario_id:usu.id, primer_nombre:primer_nombre.trim(),
          primer_apellido:primer_apellido.trim(), departamento_id:3,
        })
